...
void GetStockPool(Group& Beat, Group& Meet, Group& Miss, string* file)
{
	// To match the index in the for loop.
	Group StockPool[3];
	string ticker, start, end, act_eps, est_eps, act_to_est;


	//Read file.
	ifstream infile(file[0]);
	while (infile.good())
	{
		...
		if (stringToNum<double>(act_to_est) >= 5.0)
		{
			StockPool[0].Content.insert(pair<string, Stock>(ticker, Stock(ticker, start, end, act_eps, est_eps, act_to_est)));
			StockPool[0].Ticker.push_back(ticker);
		}
